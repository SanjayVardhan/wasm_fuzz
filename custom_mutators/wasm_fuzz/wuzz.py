...
__seed__ = "RANDOM"
__log__ = False
__log_file__ = "/home/spektre/wasm_fuzzer/wuzz.log"
...
def wrap_wasm_in_js(output):
    js = "var wasm_code = new Uint8Array(["
    for it in output:
        js += str(int(it))
        js += ','
    ...
    js = js[:-1]
    js += "]);\n"
    js += "var wasm_module = new WebAssembly.Module(wasm_code);\n"
    js += "var wasm_instance = new WebAssembly.Instance(wasm_module);\n"
    js += "var f = wasm_instance.exports.main;\n"
    js += "f();\n"
...
def fuzz(buf, add_buf, max_size):
    ...
    """
    try:
        wat_file_content = buf
        # byte array to string
        wat_file_content = wat_file_content.decode("utf-8")
        log(wat_file_content)
        function_bodies, starts, ends = extract_function_bodies(wat_file_content)
        log(f"Function Bodies: {function_bodies}")
...
# Re    construction of the wat file from the binary tokens
        ...
        stack_repair = sra2.stack_repair_algorithm(wat_lines, stack_effects)
        log(stack_repair)
        # print("lol")
        # print(stack_repair)
        # print(wat_lines)
        ...
        log(updated_wat_content)

        mutated_stuff = bytearray(updated_wat_content, "utf-8")
        return mutated_stuff
    except Exception as e:
        log(f"Exception: {e}")
        return buf

# def post_mutate(wat):
#     js = wrap_wasm_in_js(wat)
#     return js
...
#     in_1 = bytearray("AAAAAAAAAAAAAAAAAAAAAA", "utf-8")
#     in_2 = bytearray("BBBBBBBBBBBBBBBBBBBBBBBB", "utf-8")
#     out = fuzz(in_1, in_2, max_size=1024)
#     print(out)
